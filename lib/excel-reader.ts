...
/**
 * Lee un archivo Excel/CSV de Admisiones Clínicas con el formato específico:
 * - Filas 1-9: Metadata/texto basura (ignorar)
 * - Fila 10: Headers (índice 9)
 * - Desde fila 11: Datos (índice 10)
 */
export async function readExcelFileAdmisiones(file: File): Promise<ExcelData> {
  try {
    const arrayBuffer = await file.arrayBuffer()
    const workbook = XLSX.read(arrayBuffer, { type: 'array' })
    ...
    throw new Error('Error al leer el archivo Excel de Admisiones Clínicas: ' + String(error))
  }
}
