...
import {
  ...
  LiquidacionGuardiaInsert,
  ValorConsultaObraSocial,
...
export async function procesarExcelPediatria(
  ...
  try {
    ...
    // 2. Cargar valores de consultas pediátricas (ambos tipos)
    const { data: valoresData } = await supabase
      .from('valores_consultas_obra_social')
      .select('*')
      .in('tipo_consulta', ['CONSULTA DE GUARDIA PEDIATRICA', 'CONSULTA PEDIATRICA Y NEONATAL'])
      .eq('mes', mes)
    ...
    const { data: gruposData } = await supabase
      ...
      .select('*')
      .eq('mes', mes)
      .eq('anio', anio) as { data: PediatricGroupsConfig[] | null }

    const gruposMap = new Map<string, 'GUARDIA_ESTANDAR' | 'ESPECIALISTA'>()
    if (gruposData) {
      gruposData.forEach(g => {
        gruposMap.set(g.doctor_id, g.group_type)
      })
    }
