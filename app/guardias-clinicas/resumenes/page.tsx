      ...
      <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 animate-in slide-in-from-top-4 duration-700">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-400 text-xs font-bold tracking-widest uppercase">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex bg-white/5 backdrop-blur-md border border-white/10 rounded-full p-1.5 gap-2">
              ...
              <input
                ...
                className="bg-black border border-white/10 rounded-full w-24 py-1.5 px-4 text-sm font-bold focus:border-pink-500/50 outline-none"
              />
            </div>
          </div>
        </div>

        {/* Tabs Premium GrowLabs */}
        <div className="flex overflow-x-auto gap-2 p-1.5 bg-white/5 backdrop-blur-md border border-white/10 rounded-full w-fit mb-12 animate-in fade-in slide-in-from-bottom-2 duration-1000 no-scrollbar">
          <button
            ...
            className={`px-8 py-3 rounded-full font-black text-xs tracking-tighter transition-all flex items-center gap-2 ${tabActiva === 'medicos'
          ...
          <button
            ...
            className={`px-8 py-3 rounded-full font-black text-xs tracking-tighter transition-all flex items-center gap-2 ${tabActiva === 'prestadores'
              ? 'bg-pink-600 text-white shadow-[0_0_20px_rgba(236,72,153,0.3)]'
              : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-3 rounded-full font-black text-xs tracking-tighter transition-all flex items-center gap-2 ${tabActiva === 'historial'
              ? 'bg-pink-600 text-white shadow-[0_0_20px_rgba(236,72,153,0.3)]'
              : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-3 rounded-full font-black text-xs tracking-tighter transition-all flex items-center gap-2 ${tabActiva === 'excel'
              ? 'bg-pink-600 text-white shadow-[0_0_20px_rgba(236,72,153,0.3)]'
              : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
          >
        ...
        {tabActiva === 'historial' ? (
          ...
          <div className="space-y-6">
            {resumenesPorPrestador.length === 0 ? (
              ...
              <>
                {/* Grupo 70% */}
                {resumenesPorPrestador.filter(r => r.grupo_tipo === 'GRUPO_70').length > 0 && (
                  <div
                  ...
                  >
                    <h2 className="text-2xl font-bold text-pink-400 mb-4">Grupo 70%</h2>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {resumenesPorPrestador
                            .filter(r => r.grupo_tipo === 'GRUPO_70')
                            .map((resumen, idx) => (
                ...
                {/* Grupo 50% */}
                {resumenesPorPrestador.filter(r => r.grupo_tipo === 'GRUPO_50' || (r.grupo_tipo as any) === 'GRUPO_40').length > 0 && (
                  <div
                    className="rounded-xl p-6 mb-8"
                    style={{
                      background: 'rgba(255, 255, 255, 0.05)',
                  ...
                  >
                    <h2 className="text-2xl font-bold text-pink-400 mb-4">Grupo 50%</h2>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {resumenesPorPrestador
                            .filter(r => r.grupo_tipo === 'GRUPO_50' || (r.grupo_tipo as any) === 'GRUPO_40')
                            .map((resumen, idx) => (
                              <tr key={idx} className="border-b border-gray-800 hover:bg-gray-800/50">
                                <td className="px-4 py-3 text-sm text-gray-300 font-medium">{resumen.medico_nombre}</td>
                                <td className="px-4 py-3 text-sm text-gray-300 text-right">{resumen.cantidad}</td>
                ...
                {/* Otros / Sin Grupo */}
                {resumenesPorPrestador.filter(r => !r.grupo_tipo || (r.grupo_tipo as any) === 'GRUPO_40').length === 0 &&
                  resumenesPorPrestador.filter(r => r.grupo_tipo === 'GRUPO_70').length === 0 &&
                  resumenesPorPrestador.filter(r => r.grupo_tipo === 'GRUPO_50').length === 0 && (
                    <div className="text-center py-4 text-gray-400">No hay datos agrupados disponibles</div>
                  )}

                {resumenesPorPrestador.filter(r => !r.grupo_tipo).length > 0 && (
                  <div
                    className="rounded-xl p-6 mb-8"
                    style={{
                      background: 'rgba(255, 255, 255, 0.05)',
                  ...
                  >
                    <h2 className="text-2xl font-bold text-yellow-500 mb-4">Sin Grupo Asignado</h2>

                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-700">
                            <th className="px-4 py-3 text-left text-sm font-semibold text-yellow-500">Prestador</th>
                ...
                >
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-700">
                          <th className="px-4 py-3 text-left text-sm font-semibold text-pink-400">Resumen General</th>
                          <th className="px-4 py-3 text-right text-sm font-semibold text-pink-400">Cantidad</th>
                          <th className="px-4 py-3 text-right text-sm font-semibold text-pink-400">Total Bruto</th>
                          <th className="px-4 py-3 text-right text-sm font-semibold text-pink-400">Neto Consultas</th>
                          <th className="px-4 py-3 text-right text-sm font-semibold text-pink-400">Total Final</th>
                      ...
                      <tbody>
                        <tr className="font-bold">
                          <td className="px-4 py-3 text-sm text-white text-lg">TOTAL LIQUIDACIÓN</td>
